//Yiwei He yh9vhg 10/04/2021 BinarySearchTree.cpp
...
void BinarySearchTree::insert(const string& x,BinaryNode*& rootNode) {
    if (rootNode==NULL){
...
string BinarySearchTree::pathTo(const string& x,BinaryNode* rootNode)const{
    if (x < rootNode->value){
...
bool BinarySearchTree::find(const string& x,BinaryNode* rootNode)const{
    if (rootNode == NULL) 
        return false;
    else if (x < rootNode->value)
...
int BinarySearchTree::numNodes() const {
    return numNodes(root);// YOUR IMPLEMENTATION GOES HERE
}
int BinarySearchTree::numNodes(BinaryNode* rootNode)const{
    int num = 0;
